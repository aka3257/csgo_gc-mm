...
static void LogGCMessage(const char* direction, uint32_t msgType, const void* data, uint32_t size)
{
    Platform::Print("[GC_LOG] %s MsgType: %u, Size: %u\n", direction, msgType, size);
}
...
public:
    ...
    {
        g_lastSteamId = SteamUser()->GetSteamID().ConvertToUint64();    
        LogGCMessage("CLIENT_TO_GC", unMsgType, pubData, cubData);

        ExternalGCResponse response;
        if (ForwardToExternalServer(unMsgType, pubData, cubData, response))
        {
        ...
        {
            Platform::Print("No response from external GC for msg %u\n", unMsgType);
        }
    ...
    if (s_clientGC)
    {
        ...
        for (EventData &event : events)
        {
            ...
            case HostEvent::Message:
                ...
                LogGCMessage("GC_TO_CLIENT", static_cast<uint32_t>(event.id), event.buffer.data(), static_cast<uint32_t>(event.buffer.size()));
